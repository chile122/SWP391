        ...
        stage('Checkout') {
            steps {
                script {
                    ...
                    def jiraIssueKey = commitMessage.find(/SWP391-\d+/) // Adjust the regex to match your Jira issue key pattern
        ...
        stage('Update Jira') {
            steps {
                script {
                    ...
                    httpRequest(
                        url: "https://your-domain.atlassian.net/rest/api/2/issue/${jiraIssueKey}",
                        httpMode: 'PUT',
                        customHeaders: [
                        ...
                        requestBody: """
                            {
                                "fields": {
                                    "10038": "${status}" // Replace 'customfield_12345' with the ID of the 'Testcase Result' field
                                }
                            }
                        """
                    )
                    ...
                    httpRequest(
                        url: "https://chiltq6.atlassian.net/rest/api/2/issue/${jiraIssueKey}/attachments",
                        httpMode: 'POST',
                        customHeaders: [
                            [name: 'Authorization', value: jiraAuth], 
